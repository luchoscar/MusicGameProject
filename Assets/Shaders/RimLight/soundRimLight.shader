Shader "MusicGame/soundRimLight" 
{
	Properties 
	{
	...
	SubShader 
	{
		Pass 
		{
			...
			half4 frag (v2f i) : COLOR
			{
				...
				float3 diffuseLighting = tex2D (_MainTex, uv_MainTex).xyz * diffuseInt * _LightInt;
				
				//specular light
				float3 V = normalize(i.eyePosition);
				float3 H = normalize(L + V);
				float specularInt = pow(max(dot(H, i.normals), 0.0), 2.0);
				float3 specularLighting = tex2D (_MainTex, uv_MainTex).xyz * specularInt * _LightInt;
				...
				half rim = 1 - saturate(max(dot(normalize(i.eyePosition), i.normals), 0));
